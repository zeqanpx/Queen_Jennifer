...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Eva') && conf.FULLEVA_BUG !== 'on') {        
...
Asena.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.FULLEVA_BUG == 'on' && ((!message.jid.includes('-')) || (message.jid.includes('-') && 
...
Asena.addCommand({ pattern: 'EVAB ?(.*)', desc: fulleva_dsc, fromMe: true, usage: '.fulleva on / off' }, (async (message, match) => {
    var eva_status = `${conf.FULLEVA_BUG}`
    if (match[1] == 'on') {
        if (eva_status == 'on') {
